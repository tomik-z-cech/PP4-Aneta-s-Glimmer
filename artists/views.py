...
class TeamView(generic.ListView):
    ...
    def get(self, request, *args, **kwargs):
        artists = Artists.objects.all() # Get all artists
        ...
        for artist in artists: # Do this for each artist
            ...
            for valid_booking in valid_bookings: # For each booking that is rated perform this
                valid_bookings_count = valid_bookings_count + 1 # Increase rated bookings counter
                rating_total = rating_total + valid_booking.rating # Add score of the current booking
                rating = rating_total / valid_bookings_count # Count average rating
            if valid_bookings_count == 0: # If no rated bookings for artist
                rating = 0 # Set rating to 0 (if else statement in template to display "No reviews")
            artists_to_send.append( # Append the following dictionary to array to send to template
                {
                    "artist": artist, # Details of artist
                    "rating": rating, # Average rating
                    "rating_count": valid_bookings_count, # Amount of rated bookings
                }
            )
...
class TeamDetailView(generic.DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        style_slug = styles_this_artist.values_list("slug", flat=True) # Set a slug field for all styles with current artist
        filtered_styles = zip(style_slug, styles_this_artist) # Zip name of style and slug into this variable
        valid_bookings_count = 0 # Set bookings counter
        ...
        for valid_booking in valid_bookings: # For each rated booking do this
            valid_bookings_count = valid_bookings_count + 1 # Increase counter of rated bookings
            rating_total = rating_total + valid_booking.rating # Add score of the current booking
            rating = rating_total / valid_bookings_count # Count average rating
        if valid_bookings_count == 0: # If no rated bookings for artist
                rating = 0 # Set rating to 0 (if else statement in template to display "No reviews")
        return render( # Render template and send artist details, styles for current artist, rating and total of rated bookings
            request,
            self.template_name,
            {
